""""
Generate all manuscript figures from the pipeline outputs.

Run after main.py to produce publication-quality plots.  The script reads
intermediate CSV files saved by main.py and creates seven figures:
  fig1  - histogram of Theil-Sen slopes with decline threshold
  fig3  - scatter plots of slope vs. climatic variables
  fig4  - correlation heatmap
  fig5  - decline hotspot map (co-occurrence of declining species)
  fig6  - ROC and Precision-Recall curves for all three classifiers
  fig7  - SHAP summary plot
"""
...
if not os.path.exists(resident_agg_path):
    raise FileNotFoundError(
        ...
        "('resident_agg.to_csv(...)' after ecological filtering)."
    )
...
for name, model in models.items():
    ...
    ax1.plot(fpr, tpr, label=f'{name} (AUC = {roc_auc_val:.2f})')
    precision, recall, _ = precision_recall_curve(y_test, y_prob)
    pr_auc_val = average_precision_score(y_test, y_prob)
    ax2.plot(recall, precision, label=f'{name} (AP = {pr_auc_val:.2f})')
...
# Figure 7: SHAP summary plot (best model by PR-AUC on test set)
best_model = None
best_name = None
best_pr_auc = 0.0
for name, model in models.items():
